...
function validate()
    ...
    metadata =
        TOML.parsefile(joinpath(instances_path, "metadata.toml"))["instance"]
    ...
    for (i, file) in enumerate(files)
        if any(
            startswith(file, name_start) for name_start in to_remove_name_start
        )
        ...
        for (j, name) in enumerate(metadata_names)
            if i != j
                if metadata_names[i] == metadata_names[j]
                    println(
                        "Duplicate file name in metadata.toml: ",
                        metadata_names[i],
                    )
